...
class HtmlBlock:
    ...
    def __init__(self, btype, contents=None):
        ...
        if contents is None:
            ...
            assert isinstance(
                contents, list), 'HtmlBlock contents should be list'
    ...
    for i in range(len(smoothed_blocks)):
        if smoothed_blocks[i].btype != 'code':
            ...
            for j in range(len(cxt)):
                cxt[j] = re.sub(r'`(?P<text>[^`]+)`',
                                r'<code>\g<text></code>', cxt[j])
                cxt[j] = re.sub(r'\*\*(?P<center>[^\*]+)\*\*',
                                r'<strong>\g<center></strong>', cxt[j])
                cxt[j] = re.sub(r'\*(?P<center>[^\*]+)\*',
                                r'<i>\g<center></i>', cxt[j])
                cxt[j] = re.sub(r'==(?P<text>[^`]+)==',
                                r'<mark>\g<text></mark>', cxt[j])

                cxt[j] = re.sub(r'!\[(?P<imgtext>[^\[\]\"]+)\]\((?P<url>[^\(\)\[\]\"]+)\)',
                                r'<div><img src="\g<url>" alt="\g<imgtext>" class="img-responsive center-block"/></div>', cxt[j])
                cxt[j] = re.sub(r'\[(?P<urltext>[^\[\]\"]+)\]\((?P<url>[^\(\)\[\]]+)\)',
                                r'<a href="\g<url>">\g<urltext></a>', cxt[j])
...
def generate_html(htmlblocks, attrs):
    ...
    integrity="sha384-mll67QQFJfxn0IYznZYonOWZ644AWYC+Pt2cHqMaRhXVrursRwvLnLaebdGIlYNa" crossorigin="anonymous" onload="my_render()">
    </script>
</head>
<body>
    <div class="container">
    ...
    for block in htmlblocks:
        ...
        elif block.btype == 'title1':
            s += '<div class="text-center"><h1>' + \
                block.contents[0][1:].lstrip() + '</h1><p>' + \
                attrs['update_time'] + '</p><hr></div>'
        elif block.btype.startswith('title'):
            grade = block.btype[-1]
            s += '<h' + str(grade) + '>' + block.contents[0][int(
                block.btype[-1]):].lstrip() + '</h' + str(grade) + '>'
    ...
    s += '<div class="footer text-center">@2020 <a href="../../aboutme.html">YoungF</a></div>'
    s += '</div></body></html>'
    return s
...
if __name__ == '__main__':
    ...
    parser.add_argument('--css', type=str,
                        default="../../css/common.css", help='css file path')
    parser.add_argument('--parent_path', type=str,
                        default='../../index.html', help='parent page path')
    ...
    while True:
        ...
        while start_index != -1:
            ...
            raw = raw[:start_index] + '$$' + \
                raw[start_index+7:end_index] + '$$' + raw[end_index+3:]
            start_index = raw.find('```math')
        break
